...
| Area | Contents | Notes |
|---|---|---|
| `Complete/Australia/National (AS 1743)/Original PNGs/` | 873 PNGs | Dimensioned construction drawings from Complete/Australia/National (AS 1743), one per code (e.g. `TM2-4A.png`). ~753 px wide, dimensions legible. These ARE the "how to make" instructions. |
| `Complete/Australia/National (AS 1743)/SVGs/` | 78 SVGs | Hand-built in Illustrator. Text outlined to paths. Not at mm scale (e.g. a 600 mm sign is 64 units wide). Named descriptively, not by code. |
| `Processing/Australia/National (AS 1319)/Original PNGs/` | 57 PNGs | Mostly small raster pictograms (223 px), plus layout-rule figures (D1–D6), colour chart (A1), and shape figures. No per-sign dimensioned drawings. |
| `Processing/Australia/National (AS 1319)/SVGs/` | 16 SVGs | Danger / Prohibition / Safety text signs built from the three templates. |
| `Processing/Australia/National (AS 1744)/Australia/National (AS 1744).txt` | Text of the alphabet standard | Contains the six spacing tables (Series B, C, D, E, E Mod, F) at 100 mm cap height. |
...
### Phase 6 — Processing/Australia/National (AS 1319) (different model)
...
* International: see `International/REVIEW.md` (USA first, then NZ, then UK).
  ...
  * NZ (2026-09-05): NZTA sign-specifications register crawled with `tools/nz_crawl.py` (Russell passes the site's
    Imperva check once in the Browser pane; the tools reuse that session) into `Processing/New Zealand/National (TCD Manual)/` (`REGISTER.csv`,
...
* Layout (2026-09-06, agreed with Russell): country → jurisdiction pack → `Original …/` + `SVGs/<family>/`:
  ...
  `Processing/USA/<State>` (major states first, minor states as tickets), `Processing/New Zealand/National (TCD Manual)`, `UK/<Region>`.
